...
const EventEmitter = require('events'); //留意events内置模块的使用（跟之前的稍有差异）
...
class Miner extends EventEmitter {
  ...
  async _createBlock(tip, address) {
    ...
    const attempt = new BlockTemplate({
      ...
      version: version,
      time: time,
      bits: target, //为什么两者字面上不一致？有什么刻意的考量吗？
      locktime: locktime,
      mtp: mtp,
      flags: state.flags,
      address: address,
...
class MinerOptions {
  ...
  constructor(options) {
    this.network = Network.primary; //参见../protocol/network.js
    this.logger = null;
    this.workers = null; //此处workers默认为
    this.chain = null;
    this.mempool = null;
