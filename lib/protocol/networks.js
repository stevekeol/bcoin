...
main.seeds = [ //1.DNS种子：类似DNS服务器，返回比特币网络上完整节点IP地址，用于协助发现节点。 2.他们有什么特殊权限吗？
...
main.magic = 0xd9b4bef9; //神奇数，作为区块链的分隔标识符。可快速识别该文件/数据结构所支持的类型。
...
//1.节点在运行起来初始化的时候，检查点列表就被固定了。用高度作map索引，加快检查点的查找。
//2.作用:帮助新接入的节点选择正确的主链（保持在线的节点通过共识识别主链）;
//       便于实现硬分叉（重大升级时，指定官方支持的分支）,保证节点从网络同步正确的主链；
//       对于本地数据，可通过检查最新的检查点，如不匹配可以调整同步起点.         
main.checkpointMap = { 
...
main.halvingInterval = 210000; //间隔减半：每挖出210000个区块比特币奖励减半。
...
main.pow = {
  ...
  limit: new BN( //
    '00000000ffffffffffffffffffffffffffffffffffffffffffffffffffffffff', //默认的最大的Target，随着挖矿的进行，该target会逐步调小。每14天，该target会适度增大/缩小，以调整难度。
    'hex'
  ),
  ...
  targetTimespan: 14 * 24 * 60 * 60, //14天，会生成2016个块。每生成2016个块，节点会将生成这些区块的实际时间与14天的目标对比，根据百分比差异修改target。
  ...
  targetSpacing: 10 * 60, //平均每个区块产出时间：10分钟
  ...
  retargetInterval: 2016, //重新调整target的间隔，2016个区块。
...
main.block = {
  ...
  bip34height: 227931, //为了解决并不唯一的交易id。 bip34开启了优雅的"三阶段软分叉"
...
main.bip30 = { //历史上曾经出现过的两起，在不同区块中的交易的TxID相同的情况（91880和91722，91842和91812）
...
main.activationThreshold = 1916; // 95% of 2016 （bip34的95阶段：只有版本定义为2且包含区块高度的区块会被接受，其他两种区块都不会被接受了。此时便完成了软分叉。）
