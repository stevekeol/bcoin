...
class Network {
  ...
  constructor(options) {
    assert(!Network[options.type], 'Cannot create two networks.'); //类的内部调用类？
    ...
    this.checkpoints = [];
    this.time = new TimeData(); //TimeData类用于处理"adjusted time"。半共识的关键代码。
  ...
  init() {
    ...
    for (const deployment of this.deploys) //参见 ./network.js line:362
    ...
    for (const key of Object.keys(this.checkpointMap)) {
      ...
      const height = Number(key); //将checkPointMap的key强制设为Number型

      this.checkpoints.push({ hash, height }); //network中的checkPoints为数组
    }

    this.checkpoints.sort(cmpNode); //按区块高度排序，因为Objects.keys不能保证key的遍历顺序
  }
  ...
  static create(options) {
    ...
    if (!Network.primary)
      Network.primary = network; //创建区块链节点网络？
